...
export function MawkibPublicContent({
  ...
  const visibleGoals = goals.filter((g) => g.published && g.status !== "archived");
                  ...
                  <div className="p-5">
                    ...
                    {g.status === "active" ? (
                      <p className="mt-2 text-[11px] text-muted-foreground">
                        يمكنك المساهمة بأي مبلغ إضافي لهذا الهدف عبر صاحب الموكب، وهي منفصلة عن تبرعك
                        الشهري.
                      </p>
                    ) : null}
                  </div>
          ...
          <div className="space-y-4">
            {visiblePosts.map((p) => (
              <article key={p.id} className="surface-card overflow-hidden p-5">
                ...
                {p.images.length > 0 ? (
                  <div
                    className={`mt-4 grid gap-2 ${p.images.length > 1 ? "sm:grid-cols-2" : ""}`}
                  >
                    {p.images.map((img) => (
                      <MawkibImage
                        key={img}
